...
class MysqlGateway {
    ...
    static src(db: Db, connection?: Connection): MysqlGateway {
        ...
        if (db.options.connectionName) {
            ...
            connectionConfig = databaseConfig.getConnectionConfig(db.options.connectionName);
            if (!connectionConfig) {
                throw new DbException(`连接配置 ${db.options.connectionName} 不存在`);
            }
        } else {
            // 使用默认配置
            obj.connectionName = databaseConfig.getDefaultConfigName();
            connectionConfig = databaseConfig.getDefaultConfig();
            if (!connectionConfig) {
                throw new DbException('默认连接配置不存在');
            }
        }
